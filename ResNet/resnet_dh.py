...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        print(f'1x1 conv : {self.conv1}')
        print(f'layer idx {layer_idx} : {out.shape}')
        layer_idx += 1
        out = self.bn1(out)
        out = self.relu(out)
        ...
        out = self.conv3(out)  # out.shape -> [batch, 64, 56, 56]
        print(f'1x1 conv : {self.conv3}')
        print(f'layer idx {layer_idx} : {out.shape}')
        layer_idx += 1
        out = self.bn3(out)
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        print(f'입력 : {x.shape}')
        x = self.conv1(x)  # out_h = (224 - 7 + 2 * 3) / 2 + 1 = 112, (batch, 64, 112, 112)
        print(f'conv : {self.conv1}')
        print(f'layer idx {layer_idx} : {x.shape}')
        layer_idx += 1
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # out_h = (112 - 3 + 2 * 1) / 2 + 1 = 56
        print(f'maxpool : {self.maxpool}')
